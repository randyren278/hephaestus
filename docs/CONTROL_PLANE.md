...
`submit <job-id> <genome-id>` is the first asynchronous path and currently covers one direct offline reference run. It accepts a registered Genome with the supported `hephaestus-reference-v1` instruction document, captures its immutable run specification, and admits at most one active job. The canonical daemon remains the sole owner of the ledger, CAS, and signing key. A bounded executor runs the supervised worker and sends evidence through acknowledged messages; the writer validates and appends those messages before acknowledging progress. `job status` exposes the durable state and the latest recorded trace coordinate. `job kill` and `kill --all` request cancellation; an acknowledgement means the request was recorded, not that a child has stopped. The job becomes terminal only after the guardian confirms process-group termination. Confirmed worker failures, including wall-budget expiry and supervised I/O failure, receive signed run results and a durable failed job terminal; operator cancellation remains a separate lifecycle outcome. Freeze blocks new admissions and remains responsive while a job runs. A daemon crash closes the guardian liveness pipe, which contains the worker process group; restart reconstructs durable events and marks unfinished jobs interrupted unless a matching signed result and completed lifecycle evidence already prove the terminal outcome. The synchronous `run`, paired Arena evaluation, and selection commands retain their existing behavior; Arena evaluation is not asynchronous yet.
