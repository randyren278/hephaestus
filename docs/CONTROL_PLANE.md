...
`arena select <evaluation-id>` is an authenticated operator command. It derives the exact registered World from the rehydrated Arena evaluation evidence, then computes or reloads the deterministic selection receipt against that World's policy. The owner-only response exposes the complete receipt, including sealed-derived correctness, reliability, cost, latency, confidence interval, and policy inputs; it must remain on the operator side of the API boundary. A retry recomputes the same receipt and returns the same selection event. Startup and explicit `replay` independently rehydrate every recorded selection and compare the canonical receipt and event against the exact registered World, failing closed for orphan, forged, or noncanonical selections. A selected result is not a promotion: `invariant_gate_verified` and `promotion_eligible` remain false until the separate invariant receipt is joined to selection in a verified promotion decision. `correctness_regressions` counts paired task correctness losses; it does not implement the World's separate `maximum_regressions` invariant policy. `metrics_eligible` reports the measured confidence, Pareto, and cost gates only. The Pareto gate's latency dimension tolerates a fixed noise floor (`max(10% of parent latency, 50ms * paired task count)`) rather than comparing raw wall-clock milliseconds, so a candidate that is genuinely better is not rejected for jitter on trivial reference tasks; only a latency increase past that floor counts as a regression. Startup and replay recompute each stored receipt under the algorithm named in that receipt, so a selection recorded before this tolerant rule existed still verifies byte-identically against its own, stricter comparison.
...
`runs`, `evaluations`, and `denials` are read-only, authenticated, bounded (`limit`, default 20, maximum 200) list commands derived from verified canonical history, added so the TUI can show runs, evidence receipts, costs, and denials without a caller already knowing an ID. None of the three takes the canonical writer, so all three stay available while a job or Arena evaluation is active. `runs` merges the durable job projection with verified `run.result_recorded` history into one newest-first list of direct runs and jobs, reporting Genome, World, state, completion reason, latency, and actual cost once a result is signed; a job still in flight has no completion fields yet. `evaluations` lists Arena evaluations newest-first with their visible aggregate scores, plus, only once recorded: the selection outcome (`metrics_eligible`, estimate/lower/upper bps, and cost and latency per side — the same fields `arena select` returns), an invariant summary (`arena invariants`'s aggregate counts and contract-satisfied flag), a Forge assessment reference (assessment ID and metrics outcome only), and any Champion transition IDs whose promotion evidence cites the evaluation. It never includes sealed task IDs, inputs, expected outputs, or raw outputs, matching the boundaries `arena select` and `arena invariants` already enforce. `denials` lists recorded refusals newest-first: `control.request_rejected` audit events (an authenticated request refused before any command could be attempted, currently only an empty `request_id`) and `trace.recorded` events carrying `TraceKind::CapabilityDenied` (a runtime authority boundary denial observed during a run). Most other `ExecuteError::Rejected` outcomes are returned to the caller but are not separately ledgered as a denial distinct from the command's own audited event, so they are not listed here; this is what is currently observable in canonical history.
