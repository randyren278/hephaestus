...
The current receipt distinguishes measured `correctness_regressions` from policy `maximum_regressions`, which remains an invariant regression budget. No trusted invariant checker is wired yet, so `invariant_gate_verified` and `promotion_eligible` are always false. Selection provides reproducible measured evidence only; it cannot authorize a Champion transition.
