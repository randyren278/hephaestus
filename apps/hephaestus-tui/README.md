...
<p align="center"><img src="../../docs/assets/pixel/tui-banner.svg" width="512" height="112" alt="Pixel-art colosseum banner in the TUI's block-glyph style"></p>
...
- `Gene Bank` is read-only: a list of extracted Genes (lineage count, positive/neutral/negative transfer tally, a `CONTRADICTION` flag, and any species) built from `gene_list`; `Enter` opens a Gene's detail (origin parent/child, evidence trial count at extraction, every transfer trial and its outcome/effect estimate, its contradiction record if any, and its species) via `gene_show`. `R` refreshes, `Esc` steps back. There is no Gene-creating control here; extraction, transfer, and speciation stay operator CLI/API actions (`docs/GENE_BANK.md`).
- `Drift, Canary & Meta-eval` opens a four-way submenu, all read-only: **Drift records** (World, kind, observed delta vs. threshold) via `drift_list`/`drift_show`; **Canaries** (candidate, previous Champion, stage, and full transition history including per-stage evidence and health) via `canary_list`/`canary_show`; **Evolver strategies** (mutation prioritization, generation/candidate counts, Gene selection policy) via `meta_strategy_list`/`meta_strategy_show`; and **Meta-evaluations** (paired strategies, per-lineage promotion/trial counts, and bootstrapped quality/cost delta confidence intervals) via `meta_list`/`meta_show`. `↑`/`↓`/`j`/`k` navigate, `Enter` opens a detail, `R` refreshes, `Esc` steps back. There is no drift-recording, canary-starting/advancing, or meta-evaluate control here; those stay operator CLI/API actions (`docs/CANARY.md`, `docs/META_EVOLUTION.md`).
