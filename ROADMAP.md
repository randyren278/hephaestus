...
**Drift and canary slice**: `hephaestus drift record` cites one verified `SelectionReceipt` pairing the current Champion (parent, the baseline) against another Genome and records a `drift.recorded` event only when the requested kind (latency, cost, correctness, or workload/reliability) crosses a fixed, documented basis-point threshold; it never touches the Champion. `hephaestus canary start` binds a candidate to the existing Forge assessment used as its shadow evaluation (never a promotion). `hephaestus canary advance` moves a canary through 5/25/50/100% on the same kind of verified evidence: healthy evidence advances, or at 100% completes by calling `champion::champion_transition_payload`'s existing, unchanged `Promote` policy with that same assessment; regressed evidence at any stage automatically aborts instead, with the Champion untouched throughout. `hephaestus canary live-check`, once a canary has completed, submits fresh evidence pairing the previous Champion against the live one; a regression automatically appends the existing, unchanged `Rollback` transition. Freeze refuses advancement but not abort or rollback. All of it is canonical, idempotent, replay-verified ledger events with CLI, protocol, and TS coverage; see [docs/CANARY.md](docs/CANARY.md), including what an in-process test can and cannot demonstrate about a live regression without relying on timing noise. This also closes the automatic half of item 8's rollback invariant: the transition an injected live regression triggers is the same `champion.transitioned` rollback that reconstructs the previous Champion, now reachable without an operator's hand on the trigger.
