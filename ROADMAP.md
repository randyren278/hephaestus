...
**Slice done**: `SupervisedRuntime::provider`/`provider_guarded` (`crates/hephaestus-runtime`) are real `RuntimeAdapter`s for Codex and Claude Code, with invocation flags pinned to the current documented non-interactive CLI contracts (docs/RUNTIMES.md cites sources), running under the same isolated worktree/environment/capability-token/budget machinery as the reference worker. A Genome's `model.provider` selects the adapter for the daemon's `run`, synchronous `RunEvaluation`, `submit` (async job), and `arena evaluate` (paired trial) paths end to end, proven by daemon-level tests against fake `codex`/`claude` binaries — including a mixed paired Arena evaluation (reference parent, provider candidate), selection over it, and a cancelled provider `submit` job. Adapter-specific `resume` is not implemented (fails closed, `RuntimeError::Unsupported`, matching the reference worker's own precedent). `submit`'s job record and Arena's admission record each bind a versioned environment identity (`reference-v1.<digest>`/`provider-v1.<digest>`, see docs/RUNTIMES.md "Hosted drivers") and a cost budget bounded by the Genome's registered World Law; a mixed Arena pair is admitted only when the World's Law sets `allow_mixed_environments`. No real provider CLI or credential has been invoked; docs/RUNTIMES.md documents the opt-in, explicitly-marked live smoke test a human runs separately.
...
**Slice done**: `hephaestus meta strategy register|show|list` registers Evolver strategies as versioned, content-addressed Genomes (`EvolverStrategyConfig`: mutation prioritization, generation count, experiment allocation, candidate count, Gene selection); `hephaestus meta evaluate <id> --strategy-a <g> --strategy-b <g> --lineage-world <w> --lineage-genome <g> ... --lineages <n>` runs each strategy's evolve run over the named held-out base lineages through the unmodified existing evolve engine, measures Champion quality (promoted-generation count) and experiment cost (paired trials), and records a replay-verified `meta_evolution.evaluated` receipt with a deterministic bootstrap confidence interval over both deltas, in the same histogram-resampling style as the Arena selection receipt. Strategies cannot alter meta-evaluators, Laws, receipts, or budgets by construction (`EvolverStrategyConfig` has no such field); every lineage's Champion is cooperatively rolled back to its starting Genome between and after the paired runs, so the comparison is always paired on identical conditions and the meta-evaluation leaves no lasting Champion side effect. `hephaestus meta show|list` inspects receipts. See [docs/META_EVOLUTION.md](docs/META_EVOLUTION.md) for what this can and cannot show today: with only the reference-operation-flip mutation available, `mutation_prioritization`, `candidate_count`, and `gene_selection` are recorded but not yet actionable, so two strategies differing only in those fields cannot demonstrate a real efficiency difference. `meta evaluate` also blocks its calling connection until every lineage/strategy run finishes; there is no progress-polling command yet.
